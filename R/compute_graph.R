...
#'
#' @importFrom methods setClass setGeneric setMethod new callNextMethod is .hasSlot initialize show
#' @name compute_graph
NULL
...
#'   looking caption.
#' @param base_font_size Base font size (pt) for node labels; the title uses
#'   `base_font_size + 4`.
#' @param label_wrap_width Character width at which node labels are wrapped.
#' @param min_width_in,max_width_in Minimum and maximum plot width, in
#'   inches. Width scales with `width_per_node_in` between these bounds.
#' @param min_height_in Minimum plot height, in inches; height also scales
#'   with the number of pipeline layers.
#' @param width_per_node_in Width, in inches, added per node along the
#'   graph's horizontal extent, before clamping to `min_width_in`/`max_width_in`.
#' @param dpi Resolution (dots per inch) of the saved PNG.
#' @export
print_pipeline <- function(
  ...
  p <- ggraph::ggraph(layout) +
    ...
    ggraph::geom_node_label(
      ggplot2::aes(label = .data$label_wrapped, fill = I(.data$color)),
      colour = "grey10",
...
#' @param title Chart title. Defaults to the `pipeline.study_name` option, or
...
#' @param x A list of FilePath/FileUses/FileOutputs objects.
#' @param ... Additional arguments passed to \code{\link{as_pipeline_graph}}.
#' @return A tbl_graph object.
...
pipeline_summary <- function(object) {
  ...
  stage_summary <- nodes_df |>
    ...
    dplyr::summarise(
      Reports    = sum(.data$artifact_role %in% c("report_source", "deliverable_report"), na.rm = TRUE),
      Data_Files = sum(.data$artifact_role %in% c("raw_data", "derived_data", "enhanced_data", "intermediate_data"), na.rm = TRUE),
      Helpers    = sum(.data$artifact_role == "helper_script", na.rm = TRUE),
      Total      = dplyr::n(),
      .groups    = "drop"
    )
...
#' @rdname pipeline_summary
#' @export
summarize_pipeline <- pipeline_summary

#' S3 summary method for tbl_graph objects
#' @param object A tbl_graph object.
#' @param ... Additional arguments (unused; for S3 method consistency).
#' @return A tibble of the stage breakdown if `object` is a pipeline graph
#'   (see \code{\link{pipeline_summary}}), otherwise the default
#'   \code{summary()} for the next applicable class.
#' @export
...
join_pipelines <- function(graph_a, graph_b, ...) {
  tg_a <- if (inherits(graph_a, "tbl_graph")) graph_a else tidygraph::as_tbl_graph(graph_a)
  tg_b <- if (inherits(graph_b, "tbl_graph")) graph_b else tidygraph::as_tbl_graph(graph_b)
