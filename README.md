...
- **`Internal_Protected_DataStore`**: Temple CBE’s protected data store and internal DB
  access layer. Re-exports every function it used to own locally
  straight from `TempleCBE` (`Internal_Protected_DataStore/R/reexports.R`), so nothing that
  calls `Internal_Protected_DataStore::<function>()` needed to change.
- **`PI_i`**: Dr. PI_i’s injury-phenotype research project —
...
- **`Project_PI_j`**: the PI_j eDerri project codebase; may hold
