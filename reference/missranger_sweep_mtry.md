...
Runs \[missRanger::missRanger()\] once for every candidate \`mtry\`,
then keeps, \*\*for each column independently\*\*, the imputation from
whichever \`mtry\` minimised that column's out-of-bag prediction error.
The \`ranger\`-based counterpart to \[missforest_sweep_mtry()\], with
the same return shape so the two engines can be compared on one data
set.
...
- data:
  ...
  remaining column that is entirely \`NA\` is refused with an error.
...
- mtry_values:
  ...
  Integer vector of \`mtry\` values to sweep. Defaults to
  \`1:missranger_max_mtry(data)\`. Note this is \*\*not\*\* \`1:(p -
  1)\`, the \`missForest\` bound – see \[missranger_max_mtry()\] for
  why, and expect a shorter sweep than \[missforest_sweep_mtry()\]
  performs on the same data.

- num.trees, pmm.k, maxiter, num.threads:

  Passed to \[missranger_oob_by_mtry()\].

- seed:

  Integer seed passed to every fit in the sweep, or \`NULL\` (default)
  for an unseeded sweep. See Reproducibility.
...
- \`oob_error\`:
  ...
  A tibble of every imputed column's OOB error at every swept \`mtry\`.

- \`best\`:

  The winning row per imputed column.
...
The same \`seed\` is passed to every fit in the sweep. That is
deliberate: holding the seed fixed across \`mtry\` values is a
common-random-numbers comparison, so differences in OOB error reflect
\`mtry\` rather than sampling noise between runs. Because \`missRanger\`
seeds itself, a seeded sweep is reproducible independently of the
\`future\` plan and the number of workers – the property a mixed
Windows/Linux/arm64 fleet needs – without depending on L'Ecuyer stream
management. \`furrr\`'s own parallel-safe seeding is still enabled,
which covers the unseeded case.
...
Set a plan before calling. Note that \`num.threads\` defaults to \`1\`
so that \`ranger\`'s own threading does not nest inside the sweep; see
