...
`TempleCBE` is the public half of a split that used to be tangled together in two private repos: **`datasci`** (Temple CBE's general-purpose analysis tooling, plus protected patient data it should never have been mixed with) and **`PI_i`** (Dr. PI_i's injury-phenotype research project, which had independently built its own general-purpose functions — `step_famd`, `glmnet_IBS`, `proc.pca`, `corr.test.all` — because there was nowhere public to put them).

The goal: pull every domain-agnostic function out of both into one clean, publicly auditable package, so `datasci` and `PI_i` can stay private (they may touch protected data) while still being *validated* against public, testable code instead of each maintaining their own private copies nobody else can check.
...
- **License is dual `GPL-3 | MIT`**, matching `pslongSim`, `omop-duck-db`, and `ML-PScore` — a deliberate ecosystem-wide choice, not a `datasci`/`PI_i`-inherited default (those stay plain MIT since they're permanently private).
