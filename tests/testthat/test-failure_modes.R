...
test_that("failure-mode: malformed and truncated archives in renv bootstrap fail closed, not with a crash", {
  ...
  # This asserts the fail-closed (NULL) contract renv fixed upstream in
  # https://github.com/rstudio/renv/commit/532d48d6303d88900aa11aac3a0a7f339466156d
  # ("safely handle invalid git tarball when extracting sha1 in bootstrap");
  # it will start failing again if a future `renv::activate()` vendors a
  # pre-fix renv release, which is a signal to re-check upstream's release notes.
  ...
  # The extracted range now defines both the wrapper and its `_impl` helper;
  # eval() into a scratch environment and pull the wrapper out by name, since
  # eval()'s return value would otherwise be whichever function is defined last.
  # `catf` is renv's own logging helper, defined elsewhere in activate.R; stub
  # it out since only the wrapper's NULL-on-failure return value is under test.
  extract_env <- new.env()
  extract_env$catf <- function(...) invisible(NULL)
  eval(parse(text = paste(act_lines[s_idx:e_idx], collapse = "\n")), envir = extract_env)
  extract_fn <- extract_env$renv_bootstrap_git_extract_sha1_tar
  ...
  expect_null(suppressWarnings(suppressMessages(extract_fn(small_tar))))

  # 3. Corrupt archive (non-gzip, junk bytes > 563 bytes)
  junk_tar <- file.path(tmp_dir, "junk.tar.gz")
  writeBin(rep(as.raw(65), 600), junk_tar)
  expect_null(suppressWarnings(suppressMessages(extract_fn(junk_tar))))
})
