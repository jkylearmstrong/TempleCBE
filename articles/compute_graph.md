...
### Pattern 1: Project-level orchestration with `MakeComputeGraph.R` (Wolfson study pattern)

In production biostatistical pipelines and collaborative studies
(modeled after the architecture in the Wolfson study and `TempleCBE`),
best practice is to encapsulate the entire project dependency structure
in a centralized orchestration script named `MakeComputeGraph.R`.

Rather than hardcoding ad-hoc graphs in interactive sessions,
`MakeComputeGraph.R` provides a single source of truth for the entire
study lifecycle:

1.  **Configures study stages & visual identity**: Uses
    [`pipeline_config()`](https://jkylearmstrong.github.io/TempleCBE/reference/pipeline_config.md)
    to establish human-readable labels and color coding for every
    analytical phase.
2.  **Declares raw & derived assets**: Uses
    [`FilePath()`](https://jkylearmstrong.github.io/TempleCBE/reference/FilePath.md)
    with semantic roles (`raw_data`, `derived_data`) so analysts know
    which files are source registries vs. derived caches.
3.  **Connects executable reports & vignettes**: Uses
    [`create_qmd_renderer()`](https://jkylearmstrong.github.io/TempleCBE/reference/create_qmd_renderer.md)
    to link reports to their explicit data dependencies and to each
    other in topological order.

`TempleCBE` ships with this exact architecture in
`vignettes/MakeComputeGraph.R` (and `inst/scripts/MakeComputeGraph.R`),
orchestrating all four analytical vignettes:
...
When this script is run, calling
[`pipeline_summary()`](https://jkylearmstrong.github.io/TempleCBE/reference/pipeline_summary.md)
on the resulting pipeline immediately yields a structured governance
audit across all project stages:

``` r

mcg_path <- if (file.exists("MakeComputeGraph.R")) {
  "MakeComputeGraph.R"
} else if (file.exists("vignettes/MakeComputeGraph.R")) {
  "vignettes/MakeComputeGraph.R"
} else {
  system.file("scripts", "MakeComputeGraph.R", package = "TempleCBE")
}

if (nzchar(mcg_path) && file.exists(mcg_path)) {
  source(mcg_path)
  temple_pipe <- make_temple_compute_graph()
  pipeline_summary(temple_pipe)
}
#> ========================================================================
#>                       Computational Pipeline Summary                    
#> ========================================================================
#> Total Nodes: 12 | Total Edges: 12 | Stale Nodes: 6 (Needs Re-render)
#> 
#> Artifact Roles:
#>   - deliverable_report        :  4
#>   - derived_data              :  2
#>   - raw_data                  :  2
#>   - report_source             :  4
#> 
...
#>              stage Reports Data_Files Helpers Total
#>             01_EDA       2          2       0     4
#>     02_Survival_CV       2          0       0     2
#>   03_Compute_Graph       2          0       0     2
#>  04_SAS_Validation       2          2       0     4
#> ========================================================================
```
