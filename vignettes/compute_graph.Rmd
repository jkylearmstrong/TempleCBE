...
`graph_intersect()`, and `graph_subtract()` (see `?database_setops`)
answer that directly, working on any graph object — an `igraph`, a
`tbl_graph`, or the `igraph` this package's own `as_igraph()` produces.
...
not a bug to work around — it's the documented contract of
`database_intersect()`/`database_subtract()`: attribute values always
come from the first graph argument, never merged with the second. For a
pipeline graph, whose nodes carry exactly this kind of node-specific
metadata (`stage`, `mtime`, `description`), that choice determines what
you see when you compare snapshots, so pass the more recent one first.

This is a genuinely different operation from `join_pipelines()`
(Pattern 5, above): `join_pipelines()` *unions* two graphs and
**coalesces** their attribute columns so neither side's data is lost —
built for combining complementary sub-pipelines maintained by different
analysts. `graph_intersect()`/`graph_subtract()` never merge; they narrow
down to which nodes/edges exist where, taking attribute values from one
side only. Reach for `join_pipelines()` to build one combined graph out
of non-overlapping pieces, and for `graph_intersect()`/`graph_subtract()`
to compare two versions of the *same* pipeline over time.
