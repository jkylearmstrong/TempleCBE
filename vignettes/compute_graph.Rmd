...
vignette: >
  %\VignetteIndexEntry{03. Visualizing Computational Pipeline Dependencies}
...
Understanding how `TempleCBE::compute_graph` compares and contrasts with
these architectures clarifies when and why to use it.
...
5.  **Decoupled execution vs. Monolithic runner**: `targets` tightly
    ...
    `TempleCBE` decouples staleness planning (`get_render_plan()`) from
    execution: it answers *what* is stale and *in what topological
...
- **For study sections**: Focused ego-subgraphs
  (`export_subgraph()`) that isolate the neighborhood around a single
  deliverable or analytical stage.
- **For protocol deliverables & manuscripts**: Sized-to-fit static
  figures (`print_pipeline()`) with intelligent Sugiyama layout
  adjustments to prevent overlapping text labels.
- **For executive sponsors**: Stage-level collapsed diagrams
  (`collapse_by_stage()`) that condense a 90-file pipeline into a clear
  4-node study milestone overview.
...
detection from code execution: `get_render_plan()` identifies
out-of-date reports and computes their topological render order,
allowing the user, Makefile, or CI/CD runner to trigger the execution
...
## Comparing two pipeline snapshots

`as_pipeline_graph()`/`as_igraph()` answer "what does the pipeline look
like." A different, equally common question is "what changed" — between
a `MakeComputeGraph.R` run today and one from last week, or between two
analysts' working copies of the same study. `as_database()`,
`graph_intersect()`, `graph_subtract()`, and `graph_union()` (see
`?database_setops`) answer that directly, working on any graph object —
an `igraph`, a `tbl_graph`, or the `igraph` this package's own
`as_igraph()` produces.

Suppose `report`'s description gets finalized after an earlier draft:

```{r snapshot_setup}
report_yesterday <- create_qmd_renderer(
  name = "t_test_example",
  path = system.file("templates", "t_test_example.qmd", package = "TempleCBE"),
  deps = list(helper),
  file_stage = "report",
  description = "Draft t-test report (not yet finalized)"
)

pipeline_yesterday <- list(helper, report_yesterday)

g_today <- as_igraph(pipeline)
g_yesterday <- as_igraph(pipeline_yesterday)
```

Both snapshots share every node (same file paths, same names), so
`graph_intersect()` keeps all of them — but whose `description` comes
through?

```{r snapshot_intersect}
as_database(graph_intersect(g_today, g_yesterday))$nodes |>
  select(name, description)
```

Today's description survives; yesterday's is silently discarded. That's
not a bug to work around — it's the documented contract of `dplyr::intersect()`/
`dplyr::setdiff()` on a `cbe_database` (see `?database_setops`): attribute
values always come from the first graph argument, never merged with the
second. For a pipeline graph, whose nodes carry exactly this kind of
node-specific metadata (`stage`, `mtime`, `description`), that choice
determines what you see when you compare snapshots, so pass the more
recent one first.

`graph_subtract()` answers "what's new since yesterday":

```{r snapshot_subtract}
as_database(graph_subtract(g_today, g_yesterday))$nodes
```

Nothing here, since today and yesterday have identical node sets in this
toy example — a real `MakeComputeGraph.R` diff would surface any newly
added `FilePath`/`FileOutputs` nodes instead.

`graph_union()` is the odd one out: rather than taking attribute values
from one side only, it **coalesces** them, so neither snapshot's data is
silently dropped on overlap:

```{r snapshot_union}
as_database(graph_union(g_today, g_yesterday))$nodes |>
  select(name, description)
```

This is the same union-and-coalesce behavior as `join_pipelines()`
(Pattern 5, above) — `join_pipelines()` is `tbl_graph`-specific and keys
on `name`, while `graph_union()` works on any graph type via
`as_database()` and lets you choose the join key with `node_by`. Reach
for `join_pipelines()`/`graph_union()` to build one combined graph out of
complementary, non-conflicting pieces, and for `graph_intersect()`/
`graph_subtract()` to compare two versions of the *same* pipeline over
time, where you specifically want one side's values to win rather than
merge.
...
In production biostatistical pipelines and collaborative studies (modeled after the architecture in the Wolfson study and `TempleCBE`), best practice is to encapsulate the entire project dependency structure in a centralized orchestration script named `MakeComputeGraph.R`.

Rather than hardcoding ad-hoc graphs in interactive sessions, `MakeComputeGraph.R` provides a single source of truth for the entire study lifecycle:

1. **Configures study stages & visual identity**: Uses `pipeline_config()` to establish human-readable labels and color coding for every analytical phase.
2. **Declares raw & derived assets**: Uses `FilePath()` with semantic roles (`raw_data`, `derived_data`) so analysts know which files are source registries vs. derived caches.
3. **Connects executable reports & vignettes**: Uses `create_qmd_renderer()` to link reports to their explicit data dependencies and to each other in topological order.

`TempleCBE` ships with this exact architecture in `vignettes/MakeComputeGraph.R` (and `inst/scripts/MakeComputeGraph.R`), orchestrating all four analytical vignettes:

```{r make_compute_graph_code, eval = FALSE}
library(TempleCBE)
library(dplyr)
...
When this script is run, calling `pipeline_summary()` on the resulting pipeline immediately yields a structured governance audit across all project stages:

```{r run_temple_compute_graph}
...
You can also use `list_stage_data_artifacts()` to scan directories and
catalogue derived data files automatically:
...
re-rendering all reports from scratch, use `get_render_plan()` to drive
selective rendering in a build script or CI/CD workflow:
...
`tidygraph::tbl_graph`, you can audit and query your workflow using
standard tidyverse grammar:
...
sub-pipeline. Use `join_pipelines()` to combine them into an overarching
study graph without duplicating shared nodes:
